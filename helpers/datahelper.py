...
class FireDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Data loader wants an idx to get the image and label paths in every step
        ...
        orig_w, orig_h = img.size # after 640 x 640 Letterbox
        tensor, scale, pad_x, pad_y = preprocess(img, self.img_size) # normalize image returns numpy
        xyxy, labels = yolo_to_xyxy(boxes, orig_w, orig_h, scale, pad_x, pad_y, self.img_size)

        image = torch.from_numpy(tensor)
        target = {
            "boxes": torch.tensor(xyxy, dtype=torch.float32).reshape(-1, 4),  # if N=0 (0,4)  shape (2, 4): 2 row, each row has 4 values (x1, y1, x2, y2)
            "labels": torch.tensor(labels, dtype=torch.int64),                 # 1 fire, 2 smoke
            "image_id": torch.tensor([idx]),  # for evaluation image_id is used to match predictions with ground truth
        }
